        ...
        <div className="px-4 sm:px-6 pt-4 sm:pt-5 pb-3 glass-panel border-b border-gray-200 rounded-b-xl relative z-[2147481000] overflow-visible">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-3 lg:gap-4">
            <SearchBar
              ...
              placeholder={t(
                "Search by name, council, pastorate, qualification...",
                "பெயர், கவுன்சில், பாஸ்டரேட், தகுதி மூலம் தேடவும்..."
              )}
              className="w-full lg:flex-1"
            />

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full lg:w-auto lg:flex lg:flex-row">
              {APPOINTMENT_REPORT_ENABLED ? (
                <Button
                ...
                >
                  {showAppointments
                    ? t("Hide Appointment Report", "நியமன அறிக்கையை மறை")
                    : t("Appointment Made", "நியமனம் செய்யப்பட்டவை")}
                </Button>
              ) : null}
              <Button
                className="bg-blue-500 hover:bg-blue-600 text-white w-full lg:w-auto"
                onClick={() => setShowDashboard(true)}
                disabled={loading || !!error || !filteredCandidates.length}
              >
              ...
              <Button
                ...
                disabled={loading || !!error || !filteredCandidates.length || downloadingPdf}
              >
                {downloadingPdf ? t("Preparing PDF...", "PDF தயாராகிறது...") : t("Download PDF", "PDF பதிவிறக்கம்")}
              </Button>
            </div>
          </div>

          {APPOINTMENT_REPORT_ENABLED && showAppointments ? (
            <div className="mt-4">
              <AppointmentReport
